...
[assembly: AssemblyDescription("Plate-solves saved LIGHT frames under the NINA image folder for an observation window (FITS UTC) and writes drift HTML (Chart.js) with optional NINA log markers.")]
[assembly: AssemblyMetadata("ShortDescription", "Plate-solved drift HTML from LIGHT frames in your NINA image directory.")]
[assembly: AssemblyMetadata("LongDescription", @"SeeDrift finds LIGHT FITS under your NINA image file path using the same LIGHT folder layout as your Imaging file pattern in the active profile, filtered by observation time between SeeDrift Start and Stop (or a Test window — local clock in plugin options, stored as UTC). Each frame is plate-solved using your NINA plate-solve profile; drift is shown as ΔRA/ΔDec arcseconds vs the first solved frame.
