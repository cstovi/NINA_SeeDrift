...
- **NINA log correlation:** When **armed** (live capture) and on **folder import**, SeeDrift scans `%LocalAppData%\NINA\Logs` (±1 calendar day from the first frame’s date). It reads **all** `.log` files whose names match that window (not only one file per day), parses **`Starting Trigger:`** lines (center-after-drift, dither-after-exposures) and **`DirectGuider` / `SelectDitherPulse`** dither commands.
  - **Per-frame hint:** Nearest trigger to each exposure start within **45 minutes** → **SequencerLogHint** on frame tooltips.
  - **Between-frame intervals:** If a trigger falls **strictly between** the previous frame’s exposure start and this frame’s exposure start, SeeDrift draws a marker **halfway along the path** between those two points: **orange triangle** = dither interval, **cyan square** = center-only interval. Hover the marker for **NINA guider Δ** (from the log: what `SelectDitherPulse` reported in **guider units**—not the sequencer’s English description and not the same as arcsec on sky) and **measured** frame-to-frame steps in this order: **header** ΔRA/ΔDec, **derived** ΔRA/ΔDec (from pixel registration + plate scale, when available), then **cumulative-pixel** Δx/Δy.
  - **Jumps:** **JumpReason** can match the nearest trigger **or** a **log interval** on the **next** frame (dither/center between this jump and the following sub). The plot subtitle’s “matched to log” counts those cases. **Jump** diamonds also append the next frame’s interval tooltip when present.
  - **Timezone:** NINA log timestamps are usually interpreted as **local** (then converted to UTC). FITS **DATE-OBS** may be UT; if dither markers are missing, check that exposure times and log times align for your session.
