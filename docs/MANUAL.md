...
| **Concurrency** | Choose **1–16** concurrent plate solves (dropdown). On a **new** install with no saved value, the default matches **physical CPU cores** reported by Windows WMI (`Win32_Processor.NumberOfCores`, summed over sockets), capped at **16**. If WMI cannot be read, SeeDrift falls back to **`Environment.ProcessorCount`** (logical processors). Each concurrent slot gets its **own** solver instance; higher values use more RAM and CPU. Use **1** to mimic older sequential behavior. Image **downsampling** for solves comes only from **NINA Options → Plate Solve** (same profile as the Plate Solve tool)—SeeDrift does not add a separate downsample step. |
