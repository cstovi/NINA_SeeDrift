...
Each **SeeDrift Stop** or **Test report** completion adds or refreshes content in the night report under **Night report folder**. The subtitle counts **batches** (runs). When a batch spans **several** FITS **OBJECT** names, the batch **heading lists those targets in the same order they first appear** in the solved frame sequence (matching the **Target:** subsections below). The HTML shows **one drift chart and one Sequencer events block per target**; drift is zeroed at each target’s first solved frame, and sequencer rows only list intervals between **two consecutive frames of that target**. File names in rows remain the actual saved paths.
