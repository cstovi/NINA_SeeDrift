...
The report page loads **Tailwind CSS**, **Chart.js**, **Hammer.js**, and **chartjs-plugin-zoom** from CDNs (needs network the first time you open the file). The drift chart supports **wheel zoom**, **drag pan**, and **double‑click to reset zoom**. Points along the path show **start** (green) and **end** (orange); a single-frame target uses one **amber** marker. **Hover** shows the **file name** and arcsecond offsets. The **Sequencer events** table wraps long **From/To** paths so the **Detail** column keeps space.
