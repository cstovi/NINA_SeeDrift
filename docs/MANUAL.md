...
- **NINA log correlation:** When **armed** (live capture) and on **folder import**, SeeDrift scans `%LocalAppData%\NINA\Logs` (±1 calendar day from the first frame’s date). It reads **all** `.log` files whose names match that window (not only one file per day), parses **`Starting Trigger:`** lines (center-after-drift, dither-after-exposures) and **`DirectGuider` / `SelectDitherPulse`** dither commands.
  ...
  - **Between-frame intervals:** If a trigger falls **strictly between** the previous frame’s exposure start and this frame’s exposure start, SeeDrift draws a marker **halfway along the path** between those two points: **orange triangle** = dither interval, **cyan square** = center-only interval. Hover the marker for **NINA guider Δ** (from the log, in NINA guider units—not necessarily detector pixels) and **measured** frame-to-frame steps (header ΔRA/ΔDec, cumulative-pixel Δ when applicable, derived arcsec when available).
